...
from panel.libs.rackspace_cloud_files import RackspaceStorage
...
if settings.USE_RACKSPACE:
    ...
    VIDEO_STORAGE = FileSystemStorage(location='%s/video' % MEDIA_ROOT, base_url='%svideo/' % UPLOAD_ROOT)
    FILE_STORAGE = FileSystemStorage(location='%s/file' % MEDIA_ROOT, base_url='%sfile/' % UPLOAD_ROOT)
    AVATAR_STORAGE = FileSystemStorage(location='%s/picture/avatar' % MEDIA_ROOT, base_url='%spicture/avatar/' % UPLOAD_ROOT)
    COVER_STORAGE = FileSystemStorage(location='%s/picture/cover' % MEDIA_ROOT, base_url='%spicture/cover/' % UPLOAD_ROOT)
    LOGO_STORAGE = FileSystemStorage(location='%s/picture/logo' % MEDIA_ROOT, base_url='%spicture/logo/' % UPLOAD_ROOT)
    PICTURE_STORAGE = FileSystemStorage(location='%s/picture/others' % MEDIA_ROOT, base_url='%spicture/others/' % UPLOAD_ROOT)